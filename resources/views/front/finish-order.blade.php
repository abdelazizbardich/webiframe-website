        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center mb-5">
                    <h2 class="m-2">{{ __('front.finish order') }}</h2>
                    <p class="m-0">{!! __('front.Your order details have been saved, please complete the form below with your contact details so that we can contact you and confirm your order with you') !!}</p>
                </div>
                @isset($success)
                    <div class="col-12">
                ...
                <div class="col-12">
                    <div>
                        <table class="alert-success table table-bordered">
                            <tr>
                                <th>{{ __('front.Demo') }}</th>
                                <td>#{{ $order->demo->id }} <a href="{{ route('demo',$order->demo->slug) }}">{{$order->demo->title}}</a></td>
                            </tr>
                            <tr>
                                <th>{{ __('front.Domain name') }}</th>
                                <td>{{ $order->full_domain }}</td>
                            </tr>
                            <tr>
                                <th>{{ __('front.Seo') }}</th>
                                <td>{{ __('front.'.$order->seo) }}</td>
                            </tr>
                            <tr>
                                <th>{{ __('front.Website language') }}</th>
                                <td>{{ __('front.'.$order->f_lang) }}</td>
                            </tr>
                            <tr>
                                <th>{{ __('front.Second language') }}</th>
                                <td>{{ __('front.'.$order->s_lang) }}</td>
                            </tr>
                            <tr>
                                <th>{{ __('front.Newsletter subscription') }}</th>
                                <td>{{ __('front.'.$order->newsletter) }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
                <div class="col-12">
                    <form class="border rounded shadow p-3 mt-3" action="{{ route('post-order-confirmed',$order->id) }}" method="post">
                        @csrf
                        ...
                        <div class="row m-0">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input value="{{ @old('due_date') }}"  type="date" id="due-date" name="due_date" placeholder="{{ __('front.The due date of your project') }}..." class="@error('due_date') is-invalid @enderror form-control form-control-lg">
                                @error('due_date')
                                    <div class="small text-danger">{{ $message }}</div>
                                @enderror
                            </div>
